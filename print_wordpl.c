...
3) On second space or tab input, include newline command, and set "fc" to 1, to avoid double space and blank lines;
4) Consider "fc" equal 1 for space or tab, so a sequence with "space+tab" or "tab+space", avoid double new lines;
*/
...
main()
{
	...
	int c, h, fc, st; /* c = character; fc = first character ; h = history ; st = spacetab */
